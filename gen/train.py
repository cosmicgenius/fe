...
from gpt import GPT
from config import train_path, val_path, meta_path, models_path, GPTConfig, TrainConfig, TrainConfigCPU, parse_args

def main():
    args = parse_args()
    if 'device' in args and args['device'] == 'cpu':
        config = TrainConfigCPU(**args)
    else:
